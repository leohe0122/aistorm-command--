...
## LLM 原生全量战场研判升级
- [ ] 审计原生快照所需的客户、拜访、购买信号、MEDDPICC 与商机事实数据
- [ ] 实现 `adNativeAnalysis` 全量战场快照、结构化 LLM 输出与事实校验
- [ ] 将 AD 刷新改为周度/重大事实增量触发的 LLM 主引擎，规则仅作失败兜底
- [ ] 展示可折叠全局战场判断条及 AI 原生/规则来源徽章
- [ ] 完成双轨观察、成本控制、生产验收与 GitHub 同步

## 后续功能与导航收敛队列（在原生研判完成后执行）
- [ ] 将 ActionCommand 的行动建议并入客户作战台任务区域并移除独立入口
- [ ] 删除 QuickReview 独立入口，将其 0→1/1→N Review 能力并入客户/商机作战台
