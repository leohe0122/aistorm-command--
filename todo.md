...
- [ ] 验证文件夹归档后仍可预览、检索和供 AI 选择引用

## 武器库登录态生产验收
- [ ] 在登录态验证中文知识文档创建成功并可预览
- [ ] 在登录态验证新建文件夹、拖拽/菜单移动及文件夹内批量上传
- [ ] 在登录态验证已归档文档可被 AI 选择与引用

## 客户作战台与商机作战室架构重构
- [x] 审计现有客户详情、商机组件、路由和数据依赖
- [x] 编制客户级与商机级信息边界、导航和迁移蓝图
- [x] 与用户确认架构蓝图和分阶段验收标准
- [ ] 建设客户作战台：客户级概览、关键人、产品覆盖和关系态势
- [ ] 建设独立商机作战室及全屏进入、返回路径
- [ ] 迁移 MEDDPICC、Blue Sheet、Win Strategy、SPIN、AI Review 和行动工作流
- [ ] 完成数据、权限、交互和回归测试
- [x] 明确方法论证据和 AI 判断只在商机上下文呈现，避免与客户级经营视图混排
- [ ] 为每个客户级与商机级关键流程实现“AI 依据—判断—行动”可解释输出
- [ ] 以 HKT EDR 验证 AI 原生流程在 Demo 中可被完整讲清
