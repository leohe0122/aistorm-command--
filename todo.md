...
- [x] 更新生产版本探针与 Service Worker 缓存标识，使人工验收可明确识别完整 AI 引导与触发事件门控版本（v7）。

## 未知回答的行动建议（2026-08-23）
- [x] 将“明确不知道/没有”的解释结果从单纯跳题升级为按当前门控缺口给出确定性销售行动建议。
- [x] 为 E、D2、M、C1、C2、P、I、D1 与触发事件缺口配置不含方法论术语的行动建议及下一步入口追问。
- [x] 优化候选卡的确认与修正表达，但继续要求 SAM 明确确认后才写入 MEDDPICC 或其他事实层。
- [x] 回归验证未知回答不写事实、行动建议不假定客户意图且下一问持续推进。
- [x] 更新生产版本探针与 Service Worker 缓存标识至 v8，并提供未知回答行动建议的人工验收任务。

## 同阶段关键人循环追问修复（2026-08-28）
- [x] 修复 Ronald、Marcos、Felix 已依次被询问后又返回 Ronald 的 E 门控循环。
- [x] 将本轮同一门控的已问关键人与已覆盖门控作为硬性去重，只有出现明确细节缺口时才允许一次不同追问。
- [ ] 回归验证 E 临时覆盖后自动推进 D2、M 或 C1，且未确认候选始终不写入事实层。

## Win 因子关键人立场去重（2026-08-28）
- [x] 在阶段门控全部满足后，阻止 Win 因子 Power 补证重复询问本轮已覆盖的 Ronald、Marcos、Felix 立场。
- [x] 实现人物 × 主题去重；同一批已问联系人用尽后，改问未覆盖关键人或下一最弱因子。
- [ ] 用生产复现序列回归验证不再回到 Ronald，且不改变未确认候选的事实写入边界。

## 关键人主题去重正式版本（2026-08-28）
- [x] 更新生产版本探针与 Service Worker 缓存标识至 v9，并准备提供人工生产验收任务。

## AI 引导项目参与人与自动沉淀升级（2026-08-28）
- [x] 新增商机参与人表、角色枚举与 `gate_participants`，使需求挖掘先锁定项目相关人。
- [x] 将商机引导从客户全量关键人切换为已确认项目参与人；按 E/C1/C2 角色绑定可询问对象，并永久排除无关人。
- [x] SAM 明确回答“与项目无关”时自动标记该参与人 irrelevant，不生成事实候选且不再询问。
- [x] 将有效候选自动沉淀至 MEDDPICC/购买信号，候选卡改为已记录可修正，门控立即更新。
- [x] 注入当前日期，禁止询问未来会议、评审或报告的结果。
- [x] 扩展 P 采购流程识别为签字、采购中心与走流程等自然表达。
- [x] 将无态度、一致、无关、未来未发生等信息缺失表达静默跳过候选。
- [x] 为未知回答展示按缺口确定的 Command 行动建议与下一步入口，不写入客户事实。
- [x] 完成端到端自动化回归并准备人工生产验收任务；用户已确认采用方案 2 直接发布。
- [x] 用户已选择方案 2：已保存 checkpoint b4d4b460、同步 GitHub main 并发布；等待用户完成人工生产验收。
- [ ] 等待用户在正式环境完成人工验收并反馈结果；验收中仅使用已授权的真实客户事实。

## AI 引导重复上下文与 Account/Deal 隔离优化（2026-08-28）
- [x] 扩展临时门控覆盖：识别已回答的扩张机会窗口，避免对同一预算/项目规划换词重复提问。
- [x] 扩展临时门控覆盖：识别交付反馈尚未产生的未来事项，避免把未发生评审或报告当作可追问事实。
- [x] 严格隔离客户经营 Account Map 与商机 Deal Map：商机阶段引导仅使用 `STAGE_REQUIREMENTS`，并对模型输出设置 Account Map 问题拦截。
- [x] 为上述重复预算、交付反馈与题库隔离场景补充回归测试，并完成类型检查、完整测试与生产构建（22 个测试文件共 112 项通过）。
- [x] 已获用户授权：已保存 checkpoint 716b9195、同步 GitHub main 并发布；已准备人工生产验收任务。
- [ ] 等待用户在正式环境验收 Account 预算规划不重复、未来交付反馈不追问及 Deal Map 不混入 Account Map 题目，并反馈结果。

## AI 引导附件建议差异核对与补齐（2026-09-02）
- [x] 审计并补齐采购流程自然表达对 P 维度与临时门控覆盖的识别。
- [x] 将当前日期同时置入商机快照，强化不询问未来计划、预计或未发生结果的范围约束。
- [x] 扩展客户经营的临时门控覆盖，避免本轮已回答的关系、标杆或扩张题目换词重复。
- [x] 核对并收紧终止词处理；未知回答保留确定性行动建议且不写事实，不回退已上线自动沉淀规则。
- [x] 补充回归测试并完成类型检查、完整测试与生产构建（22 个测试文件共 114 项通过）；待用户授权后保存并发布。
- [x] 已获用户授权：已保存 checkpoint dc82ce1d、同步 GitHub main 并发布；已准备人工生产验收任务。
- [ ] 等待用户在正式环境人工验收 v12 的采购流程识别、日期事实边界、客户经营重复抑制与既有自动沉淀规则，并反馈结果。

## AI 引导否定性回答与重复追问修复（2026-09-02）
- [x] 将长回答的末句未知/尚未发生信号纳入终止判断，避免长度保护吞掉“目前还不清楚”。
- [x] 在事实分类前过滤“评审尚未开始、测试未开始、暂无结果/反馈”等否定性或未发生表述，禁止生成可写入候选。
- [x] 为“测试报告已到但正式评审未开始，因此尚不清楚”添加回归测试，验证不出候选；回答解释入口会进入既有行动建议与下一事实方向分支。
- [x] 完成类型检查、完整测试和生产构建（22 个测试文件共 115 项通过）；待用户授权后保存并发布。
- [x] 已获用户授权：已保存 checkpoint b042ac3d、同步 GitHub main 并发布；已准备人工生产验收任务。
- [ ] 等待用户在正式环境人工验收“正式评审未开始，因此目前不清楚”不生成候选或写入、显示行动建议且切换不同事实方向，并反馈结果。

> 自动化状态：项目参与人迁移已应用；类型检查、22 个测试文件共 110 项测试和生产构建通过。开发预览截图未带登录会话，仅显示加载态，不能替代用户的已登录端到端验收。
