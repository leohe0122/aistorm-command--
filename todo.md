...
## AI 原生 AD 指挥台重构
- [ ] 审计现有 AD 指挥台的全局 Review、客户分析、SAM 教练与行动任务数据契约
- [x] 定义首屏 AI 今日指令、异常预警和挂起确认的事实来源、判断与人类控制准则
- [ ] 实现 AI 建议的持久化确认、跳过及 AD 作战任务闭环
- [ ] 重构 AD 首屏，数据图表与 SAM 教练降级为按需展开的次级视图
- [ ] 验证 0→1/1→N 客户卡片 AI 单句结论、SAM 能力预警和 GitHub 同步
- [ ] 对事实不足客户显示“数据不足，暂不判断”，仅将有真实行动或证据的事项升级为 AD 指令

## 移除客户优先级标签
- [ ] 审计并移除 AD 指挥台、战场地图和客户作战台中的 P0/P1/P2 客户标签
- [ ] 将 AI 指挥建议从客户优先级排序改为事实、风险强度、行动时限和阶段排序
- [ ] 验证客户列表与 AI 指令不再显示或依赖 P0/P1/P2

## 下线重构前旧 Demo
- [x] 移除旧 demo.html、旧 Demo 分发入口与演示访问管理页面
- [x] 清理旧 Demo 专属截图、音频和令牌分发代码，不影响真实系统数据
- [ ] 验证旧 Demo URL 不再提供重构前的误导性内容
