...
## LLM 原生 AD 指挥研判
- [ ] 审计规则建议、全局 Review 和 SAM 教练 LLM 逻辑的可复用边界
- [ ] 对新生成的客户级与商机级建议调用 LLM，输出方法论判断和 AD 具体动作
- [ ] 每周接入一次全局战场研判与 AD 本周三件行动
- [ ] 为 SAM 教练卡增加按需生成、非持久化的 LLM 教练分析
- [ ] 完成成本控制、回归测试、生产验收与 GitHub 同步
