...
## 旧版客户展开深链接清除
- [ ] 定位并移除 `/battle-map?clientId=...` 触发旧版客户展开态的路由分支
- [ ] 将遗留客户与商机查询参数重定向至新版客户作战台或独立商机作战室
- [ ] 在生产环境验证旧链接不再展示混合详情页面
...
- [ ] 按确认的新方法论重构客户作战台和战场地图导航

## 已确认的 0→1 客户作战台落地
- [ ] 实现“系统标准动作 + SAM 自定义任务”的阶段任务中心
- [ ] 将客户级 MEDDPICC 改为证据语言与行为记录，不显示完成度或赢单得分
- [ ] 使用可追溯行为事实实现“申请开商机”门控，禁止以主观兴趣判断放行
- [ ] 进入商机后将客户作战台收敛为关系资产与商机入口，不再显示 0→1 发现工作区

## 购买信号门控重构
- [ ] 将开商机门控改为意向主体、决策链触达与触发事件三项客户端事实
- [ ] 为每项购买信号采集谁、何时、客户原话或外部事件等可追溯证据
- [ ] 将三项购买信号事实快照带入商机作战室首屏，不携带产品方案假设
- [ ] 完成测试与生产验收后同步最终代码至 GitHub main 分支供外部审核

## 战场地图发现层收敛
- [ ] 将战场地图客户卡片精简为名称、阶段、负责人、最近触达、风险提示与客户作战台入口
- [ ] 移除战场地图卡片中的 MEDDPICC、双阶段进度、AI Review、关键人和商机细节

## 前端版本刷新修复
- [ ] 修复服务工作线程的静态缓存版本策略，确保发布后不再持续加载旧版界面
