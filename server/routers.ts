...
const ACCOUNT_GUIDANCE_STAGE_BY_CUSTOMER_STAGE: Record<CustomerStage, keyof typeof ACCOUNT_REQUIREMENTS> = {
  "建图": "初步接触",
  "进门": "关系发展",
  "定痛": "关系发展",
  "找人": "关系发展",
...
function accountRequirementHasEvidence(key: string, contacts: any[], meetings: any[], signals: any[]) {
  const contactText = contacts.map(contact => `${contact.name || ""} ${contact.title || ""} ${contact.buyingRole || ""} ${contact.relationship || ""}`).join("\n");
  const evidenceText = `${contactText}\n${meetings.map(meeting => `${meeting.attendees || ""} ${meeting.keyPoints || ""}`).join("\n")}\n${signals.map(signal => `${signal.subjectName || ""} ${signal.statement || signal.sourceReference || ""}`).join("\n")}`;
  switch (key) {
    case "contactCoverage": return contacts.length > 0;
    case "contactInfluence": return contacts.some(contact => String(contact.buyingRole || "") !== "" && String(contact.buyingRole || "") !== "未知");
    case "businessPressure": return signals.some(signal => ["intent_subject", "trigger_event"].includes(String(signal.signalType || ""))) || /(合规|监管|事故|风险|预算|高层指令|降本|到期|不续签)/i.test(evidenceText);
    case "competitorPresence": return /(竞品|竞争|替换|crowdstrike|奇安信|深信服|sentinelone|defender|symantec|mcafee)/i.test(evidenceText);
    case "championActivity": return contacts.some(contact => /champion|支持者|内线/i.test(`${contact.buyingRole || ""} ${contact.relationship || ""}`)) || /(主动联系|内部信息|帮我们推动|替我们推动)/i.test(evidenceText);
    case "executiveEngagement": return /(ceo|cio|ciso|cto|cfo|vp|总裁|首席|副总裁)/i.test(evidenceText);
    case "brandPerception": return /(品牌|认可|顾虑|印象|评价|倾向)/i.test(evidenceText);
    case "blocker": return /(反对|抵触|保守|阻力|不同意|不认同)/i.test(evidenceText);
    case "deliveryFeedback": return /(交付|上线|验收|反馈|评价)/i.test(evidenceText);
    case "expansionOpportunity": return /(扩张|新增预算|规划|新项目|未来.*月)/i.test(evidenceText);
    case "referenceWillingness": return /(案例|联合推广|标杆|背书|引用)/i.test(evidenceText);
...
1. 只有阶段门控已满足或当前阶段没有定义硬门控时，才判断 Pain、Power、Champion、Value、Control 中哪一类最弱或证据最薄弱，并围绕该因子的具体事实缺口提问。
2. 若五个 Win 因子都不低于 75，最后只验证竞争防守风险：客户是否仍有 No Decision、存量厂商或无法反驳的竞品论点。
3. 问题必须引用快照中已知的具体人物或具体事件，并承接本轮临时问答；严禁泛化问“谁对方案有态度”，也不得要求 SAM 重复已经回答的内容。
...
async function buildOpportunityGuidanceSnapshot(clientId: number, opportunityId: number, history: GuidanceHistoryTurn[] = [], requestedStage?: string) {
  ...
  const missingStageRequirements = stageRequirements.map(requirement => {
    ...
    return { ...requirement, met: metByStoredFact || metByTransientAnswer, evidence, transient: metByTransientAnswer && !metByStoredFact };
  ...
  aiGuidance: router({
    ...
    interpretAnswer: protectedProcedure.input(z.object({
      ...
      if (isGuidanceTopicExhaustionAnswer(input.answer)) {
        ...
        return toAnswerExtractionResult(exhausted);
      }
      ...
      try {
        ...
        return toAnswerExtractionResult(fallback);
      }
      const raw = getLLMTextContent(result?.choices?.[0]?.message?.content);
      if (!raw) {
        const fallback = buildProvisionalAnswerCandidate(input.scope, input.question, input.answer, uncoveredMeddpiccDims, fullConversationHistory);
      ...
      try {
        ...
        if (parsed?.candidateTarget === "none") {
          return toAnswerExtractionResult(buildProvisionalAnswerCandidate(input.scope, input.question, input.answer, uncoveredMeddpiccDims, fullConversationHistory));
        }
        parsed.topicStatus = "continue";
        return toAnswerExtractionResult(parsed);
      } catch {
...
${knowledgeNote}`;
  ...
  meetings: router({
    ...
    extractFullSignals: protectedProcedure.input(z.object({
      ...
      const extractionTimeout = setTimeout(() => extractionController.abort(), 120_000);
    ...
    /**
     * 异步提取架构：先保存拜访记录并立即返回 meetingId，后台异步触发 AI 提取。
     * 避免反向代理硬超时（~60s）导致 502 HTML。
     */
    startExtraction: protectedProcedure.input(z.object({
      clientId: z.number(),
      clientName: z.string(),
      meetingDate: z.string(),
      visitType: z.string().optional(),
      ...
      Promise.resolve().then(async () => {
        ...
        try {
          const sourceText = input.transcriptText
            ...
            : `【SAM 记录】\n${input.keyPoints}`;
          let accountContext = "";
          try { accountContext = await getAccountDiagnosticContext(input.clientId); } catch (ctxErr: any) { console.error("[extractFullSignals] accountContext failed:", ctxErr?.message); }
          const prompt = `你现在是 SAM 的拜访后作战引导助手。SAM 只负责如实记录发生了什么；你必须从以下记录中一次性提取可验证事实，供 SAM 确认。\n\n客户：${input.clientName}\n日期：${input.meetingDate}\n拜访类型：${input.visitType || "拜访"}\n参会人：${input.attendees || "数据不足"}\n接触方式：${input.contactType || "数据不足"}\n发起方：${input.initiatedBy || "数据不足"}\n\n${sourceText}\n\n${accountContext}\n\n【严格规则】\n1. 只提取记录中明确出现或可逐字定位的客户事实；禁止猜测客户意图、预算、人物立场或竞争态势。\n2. 没有明确证据的数组返回空数组，字段无法确认则返回 null；不要为了填满字段而创作。\n3. suggestedScore 只能是 0/25/50/75/100，且 evidence 必须包含原话或可回溯表述。\n4. 关键人角色只在记录明确说明其决策职责或行为时填写；否则为"未知"。\n5. nextBestAction 必须是 SAM 下一次要验证的一件事，不能是产品推销动作。\n6. meetingSummary 用不超过120字概括本次已确认事实与未确认边界。\n7. 按给定 JSON Schema 返回，JSON 外不得输出任何文字。`;
          const extractionController = new AbortController();
         ...
         const result = await invokeLLM({
          ...
          clearTimeout(extractionTimeout);
          if (!result?.choices?.length || !result.choices[0]?.message) throw new Error("AI 未返回有效响应");
          const raw = getLLMTextContent(result.choices[0].message.content);
          if (!raw) throw new Error("AI 未返回可确认的拜访信号");
          let parsed: unknown;
          try { parsed = JSON.parse(extractJSON(raw)); } catch { throw new Error("AI 返回格式无效"); }
          const signals = normalizeFullMeetingSignals(parsed);
          const aiMinutes = `## 拜访作战日志\n\n${signals.meetingSummary}\n\n### 下一步需验证\n${signals.nextBestAction}`;
          if (db && id) {
            const { meetingMinutes } = await import("../drizzle/schema");
            await db.update(meetingMinutes).set({ aiMinutes, aiFullSignals: { status: "done", version: AI_NATIVE_GUIDANCE_VERSION, generatedAt: new Date().toISOString(), ...signals } as any, aiFullSignalsConfirmedKeys: [] }).where(eq(meetingMinutes.id, id));
          }
          setImmediate(() => triggerSingleClientRefresh(input.clientId));
        } catch (err: any) {
          // 提取失败：标记为 failed
          console.error("[extractFullSignals] async extraction failed:", err?.message, err?.stack?.slice(0, 300));
          if (db && id) {
            const { meetingMinutes } = await import("../drizzle/schema");
            await db.update(meetingMinutes).set({ aiFullSignals: { status: "failed", error: err?.message || "未知错误", failedAt: new Date().toISOString() } as any }).where(eq(meetingMinutes.id, id));
          }
        }
      });
      // 4. 立即返回（<500ms）
      return { meetingId: id, status: "pending" as const };
    }),
    /** 轮询 AI 提取状态 */
    getExtractionStatus: protectedProcedure.input(z.object({ meetingId: z.number() })).query(async ({ input }) => {
      const db = await getDb();
      if (!db) return { status: "failed" as const, error: "数据库不可用" };
      const { meetingMinutes } = await import("../drizzle/schema");
      const [meeting] = await db.select().from(meetingMinutes).where(eq(meetingMinutes.id, input.meetingId)).limit(1);
      if (!meeting) return { status: "failed" as const, error: "未找到拜访记录" };
      const signals = (meeting as any).aiFullSignals;
      if (!signals || typeof signals !== "object") return { status: "pending" as const };
      if (signals.status === "done") return { status: "done" as const, signals: normalizeFullMeetingSignals(signals) };
      if (signals.status === "failed") return { status: "failed" as const, error: signals.error || "提取失败" };
