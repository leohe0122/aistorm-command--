...
describe("作战工作流入口收敛", () => {
  ...
  it("更新 Service Worker 后加载新版入口脚本，避免旧缓存遮蔽 AI Review 修复", () => {
    ...
    const app = projectFile("client/src/App.tsx");
    expect(indexHtml).toContain("/sw.js?v=20260823-ai-guidance-stage-gate-enforced-v2");
    expect(serviceWorker).toContain("aistorm-command-v20260823-ai-guidance-stage-gate-enforced-v2");
  ...
  it("让 AI 在客户和商机作战台主动提出一个问题，并只在 SAM 确认后沉淀回答中的事实", () => {
    ...
    expect(guidancePanel).toContain("已切换为基础引导");
    expect(projectFile("client/public/sw.js")).toContain("aistorm-command-v20260823-ai-guidance-stage-gate-enforced-v2");
