...
describe("作战工作流入口收敛", () => {
  ...
  it("让 AI 在客户和商机作战台主动提出一个问题，并只在 SAM 确认后沉淀回答中的事实", () => {
    ...
    expect(routers).toContain("今天日期：${currentDate}");
    expect(routers).toContain("不得将计划、预计或打算当作事实");
    ...
    expect(guidancePanel).toContain("行动建议（不写入客户事实）");
    expect(guidancePanel).toContain('guide.answerFocus === "action_plan"');
    expect(guidancePanel).toContain("const factHistory = guidanceHistory");
