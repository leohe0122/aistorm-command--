...
describe("AD 指挥建议生成", () => {
  ...
  it("仅用已入库事实识别需立即处理的客户对话中断", () => {
    ...
    expect(recommendations[0]).toMatchObject({ urgency: "立即处理", clientId: 1, methodology: "0→1 作战节奏 · 购买信号验证" });
    expect(recommendations[0].facts.some(f => f.label === "最后有效对话" && f.value === "47 天前")).toBe(true);
  });

  it("没有有效对话事实基线时不把客户误判为异常", () => {
  ...
  it("优先把已确认的 AD 经营行动转为待确认指令", () => {
    ...
    expect(recommendations[0]).toMatchObject({ urgency: "立即处理", clientId: 150001, fingerprint: "ad-action-71" });
  });
  ...
  it("将停滞商机与明确的 MEDDPICC 缺口关联", () => {
    ...
    expect(recommendations[0]).toMatchObject({ opportunityId: 8, kind: "anomaly", urgency: "立即处理" });
