...
export function AIActiveGuidancePanel({ scope, clientId, opportunityId, className }: { scope: "customer" | "opportunity"; clientId: number; opportunityId?: number; className?: string }) {
  ...
  const startGuide = async () => {
    ...
    let health: Awaited<ReturnType<typeof healthCheck.refetch>>;
    try {
      health = await Promise.race([
        healthCheck.refetch(),
        new Promise<never>((_, reject) => window.setTimeout(() => reject(new Error("health_timeout")), 5_000)),
      ]);
    } catch {
      setRequestTimedOut(true);
      setCheckingHealth(false);
      toast.error("AI 引导服务检查超时。请重新尝试；若仍失败，请刷新登录会话。");
      return;
    }
    setCheckingHealth(false);
  ...
  const working = pendingGuide || interpretMutation.isPending || checkingHealth;
  ...
  return <section data-ai-active-guidance className={`rounded-2xl border border-fuchsia-400/25 bg-gradient-to-br from-fuchsia-400/[0.07] via-slate-950/70 to-slate-950/85 p-4 shadow-[0_14px_40px_rgba(192,132,252,0.08)] ${className || ""}`}>
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-start gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-fuchsia-400/15 text-fuchsia-100"><Sparkles className="h-4.5 w-4.5" /></span><div><h2 className="text-sm font-semibold text-fuchsia-50">AI 主动引导</h2><p className="mt-1 text-xs leading-5 text-fuchsia-100/55">AI 先读已入库事实，再一次只问一个最该验证的问题；SAM 只需如实回答。</p></div></div><Button type="button" size="sm" variant="outline" className="h-8 gap-1.5 border-fuchsia-400/30 bg-fuchsia-400/10 text-xs text-fuchsia-100 hover:bg-fuchsia-400/20" onClick={startGuide} disabled={working}>{pendingGuide || checkingHealth ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />}{guide ? "更新 AI 问题" : "让 AI 开始引导"}</Button></div>
