  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-cyan-400/25 bg-gradient-to-br from-cyan-400/[0.10] via-slate-950/65 to-slate-950/85 shadow-[0_16px_45px_rgba(8,145,178,0.08)]">
      <div className="flex flex-col gap-3 border-b border-cyan-300/15 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <Button size="sm" variant="outline" className="h-8 w-fit gap-1.5 border-cyan-300/30 bg-cyan-400/10 text-xs text-cyan-100 hover:bg-cyan-400/20" onClick={() => reviewMutation.mutate({ clientId, opportunityId })} disabled={reviewMutation.isPending || !hasEvidence}>
...
function ActionWorkspace({ clientId, clientName, opportunityId, opportunity }: { clientId: number; clientName: string; opportunityId: number; opportunity: any }) {
  const utils = trpc.useUtils();
  ...
  const [championAmmo, setChampionAmmo] = useState<string | null>(null);
  ...
  const championMutation = trpc.champion.generate.useMutation({ onSuccess: (data: any) => { setChampionAmmo(data.content || null); toast.success("Champion 突破话术已生成，请人工审核后使用"); }, onError: (error) => toast.error(`生成 Champion 话术失败：${error.message}`) });
  const counterFacts = (killSheets as any[])
    .filter((sheet: any) => sheet.clientId === clientId && (sheet.battleNotes || sheet.aiGeneratedTalk || sheet.aiContent || sheet.keyDiffs))
    .map((sheet: any) => ({
      id: sheet.id,
      competitorType: sheet.competitorName || "竞品待确认",
      counterAction: sheet.battleNotes || sheet.aiGeneratedTalk || sheet.aiContent || sheet.keyDiffs,
      controlPoints: sheet.keyDiffs || sheet.ourAdvantages,
    }));
  return <div className="space-y-4"><div className="rounded-xl border border-slate-700/60 bg-slate-900/40 p-4"><div className="text-sm font-semibold text-slate-100">行动闭环</div><p className="mt-1 text-xs leading-5 text-slate-500">AI 的建议必须由人审核并显式创建为任务；系统记录责任角色、截止日期和完成状态，但不自动代替团队执行。</p></div><section className="grid gap-4 xl:grid-cols-2"><article className="rounded-xl border border-violet-400/20 bg-violet-400/[0.05] p-4"><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-semibold text-violet-100">Champion 突破话术</div><p className="mt-1 text-xs leading-5 text-slate-400">基于该商机已入库的 Champion、竞争和 Win Strategy 生成，不能替代关键人行为证据。</p></div><Button size="sm" variant="outline" disabled={!opportunity?.champion || championMutation.isPending} onClick={() => championMutation.mutate({ clientId, clientName, championName: opportunity.champion, ammoType: opportunity.blueSheetCompetitor || opportunity.competitorName ? "竞品对标" : "ROI测算", notes: `${opportunity.name || "当前商机"} · ${opportunity.stage || "阶段待补"} · ${opportunity.winStrategy || "策略待补"}` })} className="h-8 shrink-0 gap-1 border-violet-400/30 text-xs text-violet-100"><Sparkles className="h-3.5 w-3.5" />生成话术</Button></div><p className="mt-3 rounded-lg border border-violet-400/15 bg-slate-950/50 px-3 py-2 text-xs text-slate-300">当前 Champion：{opportunity?.champion || "数据不足，暂不生成"}</p>{championAmmo && <div className="prose prose-invert prose-sm mt-3 max-w-none rounded-lg border border-violet-400/20 bg-slate-950/55 p-3 text-xs"><ReactMarkdown>{championAmmo}</ReactMarkdown></div>}</article>{counterFacts.length > 0 && <article className="rounded-xl border border-amber-400/25 bg-amber-400/[0.05] p-4"><div className="text-sm font-semibold text-amber-100">竞品反制动作</div><p className="mt-1 text-xs leading-5 text-slate-400">以下动作来自已入库竞争事实。确认后才会创建 POD 任务，并在任务中保留竞争事实编号。</p><div className="mt-3 space-y-3">{counterFacts.map((fact: any) => { const title = `竞品反制：${fact.competitorType || opportunity?.blueSheetCompetitor || opportunity?.competitorName || "待确认竞品"}`; const alreadyCreated = tasks.some((task: any) => task.title === title && task.description?.includes(`竞争事实 #${fact.id}`)); return <div key={fact.id} className="rounded-lg border border-amber-400/20 bg-slate-950/50 p-3"><div className="text-[10px] font-semibold tracking-wide text-amber-200">竞争事实 #{fact.id} · {fact.competitorType || "竞品待确认"}</div><p className="mt-1 text-xs leading-5 text-slate-100">{fact.counterAction}</p><div className="mt-3 flex flex-wrap items-center gap-2"><Select value={counterRole} onValueChange={value => setCounterRole(value as (typeof roleOptions)[number])}><SelectTrigger className="h-8 w-24 border-amber-400/25 bg-slate-950/65 text-[11px]"><SelectValue /></SelectTrigger><SelectContent>{roleOptions.map(role => <SelectItem key={role} value={role}>{role}</SelectItem>)}</SelectContent></Select><Button size="sm" variant="outline" disabled={alreadyCreated || createMutation.isPending} onClick={() => createMutation.mutate({ clientId, opportunityId, assignedRole: counterRole, title, description: `${fact.counterAction}\n\n来源：竞争事实 #${fact.id}；控制点：${fact.controlPoints || "数据不足"}` })} className="h-8 border-amber-400/35 text-xs text-amber-100 hover:bg-amber-400/10">{alreadyCreated ? "已创建任务" : "确认并创建 POD 任务"}</Button></div></div>; })}</div></article>}</section><div className="rounded-xl border border-cyan-400/20 bg-cyan-400/[0.05] p-4"><div className="mb-3 flex items-center gap-2 text-sm font-medium text-cyan-100"><Plus className="h-4 w-4" />新建商机行动</div><div className="grid gap-3 lg:grid-cols-[1fr_140px_160px]"><Input value={form.title} onChange={event => setForm(prev => ({ ...prev, title: event.target.value }))} placeholder="例如：确认两个最终用户部门汇报时间" className="border-slate-700 bg-slate-950/60 text-xs" /><Select value={form.role} onValueChange={value => setForm(prev => ({ ...prev, role: value as any }))}><SelectTrigger className="h-9 border-slate-700 bg-slate-950/60 text-xs"><SelectValue /></SelectTrigger><SelectContent>{roleOptions.map(role => <SelectItem key={role} value={role}>{role}</SelectItem>)}</SelectContent></Select><Input type="date" value={form.dueDate} onChange={event => setForm(prev => ({ ...prev, dueDate: event.target.value }))} className="border-slate-700 bg-slate-950/60 text-xs" /></div><Textarea value={form.description} onChange={event => setForm(prev => ({ ...prev, description: event.target.value }))} placeholder="完成标准、依赖条件或来自 AI Review 的事实依据" className="mt-3 min-h-[80px] resize-y border-slate-700 bg-slate-950/60 text-xs" /><div className="mt-3 flex justify-end"><Button onClick={() => createMutation.mutate({ clientId, opportunityId, assignedRole: form.role, title: form.title, description: form.description || undefined, dueDate: form.dueDate || undefined })} disabled={!form.title.trim() || createMutation.isPending} className="gap-1.5"><Plus className="h-3.5 w-3.5" />{createMutation.isPending ? "创建中…" : "创建任务"}</Button></div></div><div className="space-y-2">{isLoading ? <div className="py-8 text-center text-xs text-slate-500"><Loader2 className="mr-1 inline h-3.5 w-3.5 animate-spin" />加载任务…</div> : tasks.length === 0 ? <div className="rounded-xl border border-dashed border-slate-700 px-4 py-9 text-center text-xs text-slate-500">暂无该商机的行动任务。请将已审核的建议转化为明确的责任事项。</div> : tasks.map((task: any) => <article key={task.id} className="flex flex-col gap-3 rounded-xl border border-slate-700/60 bg-slate-950/55 p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="mb-1 flex flex-wrap items-center gap-2"><span className="text-sm font-medium text-slate-100">{task.title}</span><span className="rounded border border-cyan-300/20 bg-cyan-400/10 px-1.5 py-0.5 text-[10px] text-cyan-200">{task.assignedRole}</span><span className={cn("rounded px-1.5 py-0.5 text-[10px]", task.taskStatus === "done" ? "bg-emerald-400/10 text-emerald-200" : "bg-slate-700/50 text-slate-400")}>{task.taskStatus === "done" ? "已完成" : task.taskStatus === "in_progress" ? "进行中" : "待处理"}</span></div>{task.description && <p className="text-xs leading-5 text-slate-500">{task.description}</p>}<p className="mt-1 text-[10px] text-slate-600">截止：{formatDate(task.dueDate)}</p></div>{task.taskStatus !== "done" && <Button size="sm" variant="outline" onClick={() => statusMutation.mutate({ id: task.id, taskStatus: "done" })} disabled={statusMutation.isPending} className="h-8 shrink-0 gap-1.5 border-emerald-400/25 text-xs text-emerald-200 hover:bg-emerald-400/10"><CheckCircle2 className="h-3.5 w-3.5" />标记完成</Button>}</article>)}</div></div>;
}
