  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-cyan-400/25 bg-gradient-to-br from-cyan-400/[0.10] via-slate-950/65 to-slate-950/85 shadow-[0_16px_45px_rgba(8,145,178,0.08)]">
      <div className="flex flex-col gap-3 border-b border-cyan-300/15 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" className="h-8 w-fit gap-1.5 border-amber-300/30 bg-amber-400/10 text-xs text-amber-100 hover:bg-amber-400/20" onClick={() => { document.querySelector<HTMLButtonElement>('[data-room-section="actions"]')?.click(); window.setTimeout(() => document.getElementById("opportunity-material-generator")?.scrollIntoView({ behavior: "smooth", block: "start" }), 0); }}><Swords className="h-3.5 w-3.5" />生成作战材料</Button><Button size="sm" variant="outline" className="h-8 w-fit gap-1.5 border-cyan-300/30 bg-cyan-400/10 text-xs text-cyan-100 hover:bg-cyan-400/20" onClick={() => reviewMutation.mutate({ clientId, opportunityId })} disabled={reviewMutation.isPending || !hasEvidence}>
...
function OpportunityMaterialGenerator({ client, opportunity }: { client: any; opportunity: any }) {
  ...
  return <section id="opportunity-material-generator" className="rounded-xl border border-cyan-400/25 bg-cyan-400/[0.05] p-4">
...
function ActionWorkspace({ clientId, opportunityId, initialReviewId }: { clientId: number; opportunityId: number; initialReviewId?: number | null }) {
  ...
  const currentClient = (allClients as any[]).find((item: any) => item.id === clientId) as any;
  const champScore = Number((meddpiccData as any)?.championScore ?? (meddpiccData as any)?.champion ?? 0);
  const economicBuyerScore = Number((meddpiccData as any)?.economicBuyerScore ?? (meddpiccData as any)?.economicBuyer ?? 0);
  ...
  const deployRecs = useMemo(() => {
    const suggestions: Array<{ role: (typeof roleOptions)[number]; title: string; description: string }> = [];
    if (champScore < 3) suggestions.push({
      role: "SAM",
      title: "启动 Champion 培育计划",
      description: `事实依据：Champion 维度当前为 ${champScore}/4，尚未形成可验证的支持路径。请由 SAM 补录 Champion 的动机、最近互动与下一次共同动作。`,
    });
    if (economicBuyerScore < 3) suggestions.push({
      role: "AD",
      title: "覆盖 Economic Buyer 并建立高层通路",
      description: `事实依据：Economic Buyer 维度当前为 ${economicBuyerScore}/4。请由 AD 确认预算影响者，安排高层打点或资源决策沟通。`,
    });
    const isPoc = /POC|技术验证/i.test(String(opp?.stage || ""));
    if (isPoc && !opp?.assignedSaId && !saHasTask) suggestions.push({
      role: "SA",
      title: "SA 入场推进技术验证",
      description: "事实依据：商机已处于技术验证 / POC 阶段，但未分配 SA 且暂无 SA 任务。请确认验证范围、成功标准和技术决策人。",
    });
    if (!currentClient?.assignedRsmId && !rsmHasTask) suggestions.push({
      role: "RSM",
      title: "激活属地资源与商务通路",
      description: "事实依据：当前客户未分配属地 RSM，且暂无 RSM 协同任务。请确认属地渠道、招投标或商务关系是否需要介入。",
    });
    return suggestions;
  }, [champScore, economicBuyerScore, opp?.stage, opp?.assignedSaId, saHasTask, currentClient?.assignedRsmId, rsmHasTask]);
