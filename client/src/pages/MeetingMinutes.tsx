                         ...
                         {meeting.visitType && (
                          ...
                          {meeting.contactType && meeting.contactType !== "formal_meeting" && (() => {
                           const contactLabels: Record<string, { label: string; color: string }> = {
                              ...
                              customer_initiated: { label: "💬 客户联系", color: "bg-cyan-500/15 text-cyan-400 border border-cyan-500/30" },
                            };
                            const isCustomerInitiated = meeting.initiatedBy === "customer";
                            const info = contactLabels[meeting.contactType];
                            return info ? (
                              <span className={`text-[10px] px-1.5 py-0.5 rounded ${info.color}`}>
                                {isCustomerInitiated ? "⭐ " : ""}{info.label}
                              </span>
                            ) : null;
                          })()}
                       </div>
