...
export default function IntelRadar() {
  ...
  return (
    <div className="p-6">
      ...
      {/* Compliance Policy RSS Panel */}
      <div className="mt-5">
        <button
          className="w-full flex items-center justify-between px-4 py-3 bg-card border border-purple-500/20 rounded-xl hover:border-purple-500/40 transition-colors"
          onClick={() => setComplianceExpanded(v => !v)}
        >
          <div className="flex items-center gap-2">
            <Shield className="w-4 h-4 text-purple-400" />
            <span className="text-sm font-semibold text-foreground">港澳 + 东南亚合规政策动态</span>
            {complianceItems.length > 0 && (
              <span className="text-xs px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-400 border border-purple-500/20">{complianceItems.length} 条</span>
            )}
            <span className="text-xs text-muted-foreground ml-1">PCPD · PDPC · JPDP · Kominfo · NPC · CSA ...</span>
          </div>
          <div className="flex items-center gap-2">
            {fetchingCompliance && <Loader2 className="w-3.5 h-3.5 animate-spin text-muted-foreground" />}
            <Button
              variant="ghost"
              size="sm"
              className="h-6 gap-1 text-xs text-muted-foreground hover:text-foreground"
              onClick={(e) => { e.stopPropagation(); refetchCompliance(); }}
            >
              <RefreshCw className="w-3 h-3" />
              刷新
            </Button>
            <span className="text-xs text-muted-foreground">{complianceExpanded ? "▲" : "▼"}</span>
          </div>
        </button>

        {complianceExpanded && (
          <div className="mt-2 bg-card border border-purple-500/15 rounded-xl p-4">
            {fetchingCompliance ? (
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0 mt-0.5">
                        {item.link && (
                          <a href={item.link} target="_blank" rel="noopener noreferrer"
                            className="text-muted-foreground hover:text-purple-400 transition-colors" title="打开原文">
                            <ExternalLink className="w-3.5 h-3.5" />
                          </a>
                        )}
                      </div>
